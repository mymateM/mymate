...
import com.example.mymate.presentation.alarm.adapter.AlarmSpdContainerAdapter
...
class AlarmSpdFragment : Fragment() {
    ...
    fun getNoti() {
        ...
        viewModel.expNoti.observe(viewLifecycleOwner) {
            if (binding.alarmData.adapter != null) {
                binding.alarmData.adapter!!.notifyDataSetChanged()
            } else {
                binding.alarmData.adapter = AlarmSpdContainerAdapter(it)
            }
        }
    }
